...
- Use A4 page layout with high-quality styling.
- The report MUST be at least 2 pages long.
- PAGE 1: COVER & EXECUTIVE SUMMARY
  - Large colorful header with "AI CLINICAL NUTRITION INTELLIGENCE REPORT"
  - Food name in massive bold text.
  - A "NEAT SUMMARIZER" section: A beautifully designed box with a background color (#F1F8E9) containing a 200-word professional executive summary.
  - Health Score Gauge: A large, colorful CSS-based circular gauge.
  - User Profile context (Age, Goal, Activity).
- PAGE 2: DEEP CLINICAL ANALYSIS
  - Detailed Macronutrient & Micronutrient tables with colorful headers.
  - Metabolic Impact Analysis with charts (use CSS bars/divs for visualization).
  - Risk Prediction Matrix: A colorful table showing risks for Diabetes, Cardio, and Obesity.
  - Gut Health & Compatibility sections with icons (use emoji or CSS shapes).
  - "PREVIOUS HISTORY ANALYSIS" section: Detailed comparison with last 3 items.
- STYLING:
  - Use a vibrant color palette: Primary #1B5E20 (Dark Green), Secondary #43A047 (Medium Green), Accent #FB8C00 (Orange), Info #0288D1 (Blue).
  - Use professional fonts (Arial, Helvetica, sans-serif).
  - Add page breaks using CSS: 'page-break-after: always;'.
  - No external CSS, Inline CSS only.
  - Include print media CSS (@media print { .no-print { display: none; } body { margin: 0; padding: 0; } }).
  - Add a professional footer on every page with "Generated by AI Nutrition Pro | Confidential Clinical Data".
